...
void draw() {
  ...
  //fill(200, 180);
  //if(blockNum !=blockPrev){
  //  blockCur=0;
  //}else if(blockCur < blockNum){
  //  for(int i=0; i<=blockCur; i++){
  //    block(px.get(blockNum)-px.get(blockNum-i), py.get(blockNum)-py.get(blockNum-i), 0);
  //  }
  //  delay(100);
  //  blockCur++;
  //}else if(blockCur == blockNum){
  //  for(int i=blockNum; i>=0; i--){
  //    block(px.get(blockNum)-px.get(i), py.get(blockNum)-py.get(i),0);
  //  }
  //}
  //blockPrev=blockNum;
...
void moveCam() {
  ...
  float smoother = (blockNum-36)/blockNum;
  
  if (py.get(blockNum)>height*0.6) {
    cexInc = 0.2 + 0.5*smoother;
    ceyInc = 0.6 + 1.0*smoother;
    cezInc = 0 + 5*smoother;
    ccxInc = 0.1 + 0.5*smoother;
    ccyInc = 0.3 + 1.2*smoother;
  } else {
    cexInc = 0.2;
    ceyInc = 0.6;
    cezInc = 0;
  ...
  ccy = height - py.get(blockNum)*2.5*ccyInc - blockHeight*10*(1-ccyInc) +ccyMov;
  ccz = 0 + cczMov;
  cex = px.get(blockNum)*cexInc + blockLength*(1-cexInc) + cexMov;
  cey = height - py.get(blockNum)*2.4*ceyInc - blockHeight*10*(1-ceyInc) + ceyMov;
  cez = (py.get(blockNum)*cezInc + width*(1-cezInc))/tan(PI/3) + cezMov;


  if (mousePressed) {
    if (mouseX>width*0.6) {
      cexMov+= blockLength*0.05*(1+cexInc);
    } else if (mouseX<width*0.4) {
      cexMov-= blockLength*0.05*(1+cexInc);
    }

    if (mouseY<height*0.4) {
      ceyMov-= blockHeight*ceyInc;
